...
def home():
    ...
    posts=Post.query.order_by(Post.time_posted.desc()).paginate(per_page=5,page=page)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash("Account created for "+form.username.data+", You can login in now",'success')
...
def send_reset_email(user):
    ...
    msg.body= f'''To reset your password at Myblog visit the following link:
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Your password has been updated!, You can login in now",'success')
        return redirect(url_for('login'))
